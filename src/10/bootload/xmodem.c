...
/* output send request until starting receive */
		...
		)
{
	...
	while(1)
	{
		...
		if(c == XMODEM_EOT) /* receive finish */
		{
			serial_send_byte(SERIAL_DEFAULT_DEVICE, XMODEM_ACK);
			break;
		}
		else if(c == XMODEM_CAN) /* receive cancel */
		{
			return -1;
		}
		else if(c == XMODEM_SOH) /* receive start */
		{
			receiving++;
